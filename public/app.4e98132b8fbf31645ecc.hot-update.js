...
class Point {
  ...
  get position() {
    ...
    let distortionFactor = 0.7; // Change this to distort the blob more or less
...
blob.position = {
  x: 0.6,
  y: 0.7
}; // Move the blob to the right side
...
init = function () {
  ...
  let mouseMove = function (e) {
    ...
    // Get the mouse coordinates relative to the canvas center
    let mouseXRelativeToCenter = e.clientX - pos.x;
    let mouseYRelativeToCenter = e.clientY - pos.y;

    // Convert the rotation angle to radians
    let rotationRadians = Math.PI / 180 * 60; // 60 degrees clockwise

    // Calculate the adjusted mouse coordinates
    let adjustedMouseX = mouseXRelativeToCenter * Math.cos(rotationRadians) + mouseYRelativeToCenter * Math.sin(rotationRadians);
    let adjustedMouseY = -mouseXRelativeToCenter * Math.sin(rotationRadians) + mouseYRelativeToCenter * Math.cos(rotationRadians);

    // Use the adjusted mouse coordinates instead of the original ones
    let diff = {
      x: adjustedMouseX,
      y: adjustedMouseY
    };
...
//# sourceMappingURL=app.4e98132b8fbf31645ecc.hot-update.js.map