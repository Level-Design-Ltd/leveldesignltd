...
class Point {
  ...
  get position() {
    ...
    let distortionFactor = 0.4; // Change this to distort the blob more or less
...
init = function () {
  ...
  let mouseMove = function (e) {
    ...
    // Calculate the distance from the mouse position to the nearest blob point
    let minDist = Infinity;
    let nearestPoint = null;
    blob.points.forEach(point => {
      let diff = {
        x: e.clientX - point.position.x,
        y: e.clientY - point.position.y
      };
      let dist = Math.sqrt(diff.x * diff.x + diff.y * diff.y);
      if (dist < minDist) {
        minDist = dist;
    ...
    if (minDist < blob.radius && hover === false) {
      hover = true;
      angle = nearestPoint.azimuth;
    } else if (minDist > blob.radius && hover === true) {
      hover = false;
      angle = nearestPoint.azimuth;
      blob.color = null;
    }
    if (hover) {
      ...
      strength = Math.sqrt(strength.x * strength.x + strength.y * strength.y) * 0.3;
...
//# sourceMappingURL=app.e25d5c3ed2f974cef217.hot-update.js.map