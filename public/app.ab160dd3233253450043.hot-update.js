...
class Point {
  ...
  get position() {
    // Compute the undistorted position (i.e., if the blob was a perfect circle)
    let undistorted = {
      x: this.parent.center.x + this.components.x * (this.parent.radius + this.radialEffect),
      y: this.parent.center.y + this.components.y * (this.parent.radius + this.radialEffect)
    };

    // Introduce distortion based on the azimuth to create the bean shape
    let distortionFactor = 0.4; // Change this to distort the blob more or less
    let distortion = 1 + distortionFactor * Math.sin(2 * this.azimuth);

    // Apply the distortion to the position
    let position = {
...
//# sourceMappingURL=app.ab160dd3233253450043.hot-update.js.map